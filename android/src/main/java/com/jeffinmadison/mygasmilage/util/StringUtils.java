...
public class StringUtils {
    ...
    public static String getRemainingTimeFromDateAsHtmlString(long releaseDateLong) {
...
//        calendar.setTimeInMillis(releaseDateLong);
        ...
        if (delta < MINUTE_MILLIS) {
            ...
            resultString = String.format("View in <big>%d</big> seconds", seconds);
        } else if (delta < HOUR_MILLIS) {
            int minutes = (int) (delta / MINUTE_MILLIS);
            int seconds = (int) ((delta % MINUTE_MILLIS) / SECONDS_MILLIS);
            if (seconds > 0) {
                resultString = String.format("View in <big>%d</big> minutes <big>%d</big> seconds", minutes, seconds);
            } else {
                resultString = String.format("View in <big>%d</big> minutes", minutes);
            }
        } else if (delta < DAY_MILLIS) {
            int hours = (int) (delta / HOUR_MILLIS);
            int minutes = (int) ((delta % HOUR_MILLIS) / MINUTE_MILLIS);
            if (minutes > 0) {
                resultString = String.format("View in <big>%d</big> hours <big>%d</big> minutes", hours, minutes);
            } else {
                resultString = String.format("View in <big>%d</big> hours", hours);
            }
        } else if (delta < WEEK_MILLIS) {
            int days = (int) (delta / DAY_MILLIS);
            int hours = (int) ((delta % DAY_MILLIS) / HOUR_MILLIS);
            if (hours > 0) {
                resultString = String.format("View in <big>%d</big> days <big>%d</big> hours", days, hours);
            } else {
                resultString = String.format("View in <big>%d</big> days", days);
            }
        } else if (delta < MONTH_MILLIS) {
            int weeks = (int) (delta / WEEK_MILLIS);
            int days = (int) ((delta % WEEK_MILLIS) / DAY_MILLIS);
            if (days > 0) {
                resultString = String.format("View in <big>%d</big> weeks <big>%d</big> days", weeks, days);
            } else {
                resultString = String.format("View in <big>%d</big> weeks", weeks);
            }
        } else if (delta < YEAR_MILLIS) {
            int months = (int) (delta / MONTH_MILLIS);
            int weeks = (int) ((delta % MONTH_MILLIS) / WEEK_MILLIS);
            if (weeks > 0) {
                resultString = String.format("View in <big>%d</big> months <big>%d</big> weeks", months, weeks);
            } else {
                resultString = String.format("View in <big>%d</big> months", months);
            }
        } else {
